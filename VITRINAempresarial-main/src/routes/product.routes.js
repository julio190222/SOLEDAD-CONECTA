...
router.post('/', ...auth, async (req, res, next) => {
  try {
    ...
    const [r] = await pool.query(
      'INSERT INTO products (business_id,subcategory_id,name,slug,description,base_price) VALUES (?,?,?,?,?,?)',
      [biz[0].id, subcategory_id||null, name, slug, description||null, base_price]
    );
    if (variants?.length) {
